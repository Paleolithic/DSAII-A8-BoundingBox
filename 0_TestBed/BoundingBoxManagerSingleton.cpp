...
void BoundingBoxManagerSingleton::CalculateCollision(void)
{
	...
	for(int i = 0; i < m_nBoxs; i++)
	{
		for(int j = 0; j < m_nBoxs; j++)
		{
			if(i != j)
			{
				//If the distance between the center of both Boxs is less than the sum of their radius there is a collision (Check the x,y, and z positions) 
				if(glm::distance(lCentroid[i].x, lCentroid[j].x) < (m_lBox[i]->fWidthX + m_lBox[j]->fWidthX) &&
					glm::distance(lCentroid[i].y, lCentroid[j].y) < (m_lBox[i]->fWidthY + m_lBox[j]->fWidthY) &&
					glm::distance(lCentroid[i].z, lCentroid[j].z) < (m_lBox[i]->fWidthZ + m_lBox[j]->fWidthZ)){
